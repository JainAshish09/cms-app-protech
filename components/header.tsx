...
const Header: React.FC<HeaderProps> = ({ navbarData, menuLayoutConfig }) => {
    const { bgColor = '#ffffff', textColor = '#000000', items = [], showSearch = false } = navbarData || {};
    ...
    const renderItem = (item: CMSMenuItem, idx: number) => {
        switch (item.type) {
            case 'button':
                return (
                    <a key={idx} href={item.url || '#'} target={item.newTab ? '_blank' : '_self'}
            ...
            case 'text':
                return (
                    <span key={idx} className={item.controls?.fontSize || item.fontSize || 'text-base'}
            ...
            case 'logo':
                return (
                    <a key={idx} href={item.url || '#'}>
                        {item.image && <img src={item.image} alt={item.label || 'Logo'} style={{ height: `${item.height || 40}px` }} />}
                    </a>
                );
            case 'link':
            default:
        ...
        <div>
            <div className="fixed top-0 inset-x-0 z-50 py-3 px-4 sm:px-6">
                <div className="max-w-9xl mx-auto">
                    <header style={headerStyles} className="relative bg-white rounded-lg p-3 flex items-center justify-between shadow-md">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            >
                                {mobileMenuOpen ? (
                                    // X icon (Close)
                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-black" fill="none"
                    ...
                    {/* Mobile dropdown */}
                    {mobileMenuOpen && (
                        <div className="md:hidden mt-2 bg-white rounded-lg shadow p-4 space-y-1">
                            {!showQuickLinks ? (
                                <>
                                    ...
                                    <button
                                        ...
                                        className="w-full text-left px-3 py-2 bg-gray-200 rounded"
                                    >
                                        Quick Links
                                    </button>
                                </>
                    ...
                    {/* Desktop Quick Links shown when menu is open */}
                    {mobileMenuOpen && window.innerWidth >= 768 && showQuickLinks && (
                        <div className="hidden md:block bg-white rounded-lg p-0 shadow-md mt-4">
                            <MenuLayout config={menuLayoutConfig} />
                        </div>
                    )}
                </div>
